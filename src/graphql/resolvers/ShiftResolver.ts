    ...
    Query: {
        ...
        getShiftsPagination: async (_root: undefined,
            ...
            try {
                ...
                const data = await Promise.all(getShift?.data?.map(async (data: iShift) => {
                    const getTypeShiftOff = await EmployeePublicHoliday.findById(data?.timeOff).populate("title")
                    return {
                        _id: data?._id,
                        from: data?.from,
                        to: data?.to,
